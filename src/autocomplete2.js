    ...
    var bigAutocomplete = new function() {
        ...
        this.init = function() {
            ...
            $(document).bind('mousedown', function(event) {
                ...
                if ($("#bigAutocompleteContent").length == 0 && (!$target.is($(currentInputText)))) {
            ...
            $("#bigAutocompleteContent").delegate("tr", "click", function() {
                $(currentInputText).val($(this).find("div").html());
        ...
        this.autocomplete = function(param) {
                        ...
                        if (data != null && $.isArray(data)) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                for (var j = 0; j < config.key.length; j++) {
                                    strKey += data[i][config.key[j]];
                                }
                                if (strKey.indexOf(keyword_) > -1) {
                                    data_.push(data[i]);
                                }
                            }
                            makeContAndShow(config, data_);
                        } else if (url != null && url != "") { //ajax请求数据
                            $.post(url, {
                                keyword: keyword_
                            }, function(result) {
                                makeContAndShow(config, result.data)
                            }, "json")
                        }
                ...
                //组装下拉框html内容并显示
                function makeContAndShow(c, data_) {
                    ...
                    for (var i = 0; i < data_.length; i++) {
                        ...
                        for (var j = 0; j < c.value.length; j++) {
                            strKey += data_[i][c.value[j]];
                        }
                        cont += "<tr><td><div>" + strKey + "</div></td></tr>"
                    }
                    cont += "</tbody></table>";
                    ...
                    $bigAutocompleteContent.find("tr").each(function(index) {
                        $(this).data("jsonData", JSON.stringify(data_[index]));
                    })
                }
                //输入框focus事件
